...
const submitForm = async () => {
  try {
      const res = await axios.post('http://localhost:8000/api/sejour_visa/', visa )
      visa.libelle = ''
      visa.typeVisa = ''
...
const fetchVisa = async () => {
  try {
    const resvisa = await axios.get('http://localhost:8000/api/sejour_visa/')
...
// Modal modifier
  ...
  const submitEdit = async () => {
    ...
    try {
      await axios.patch(`http://localhost:8000/api/sejour_visa/${selectedVisaId.value}/`, editData)
  ...
  const submitDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8000/api/sejour_visa/${selectedVisa.value.id}/`)
