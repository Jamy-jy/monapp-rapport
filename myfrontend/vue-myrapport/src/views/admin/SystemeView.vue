        ...
        <form @submit.prevent="handleSave" class="space-y-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                <div class="sm:col-span-3">
                  <PlaceholderInput
                    v-model="system.titre_systeme"
                    label="nom incident"
                    placeholder="Entrez le nom de l'incident"
                  />
                  <p v-if="serverErrors.titre_systeme" class="text-red-500 text-sm mt-1">
                    {{ serverErrors.titre_systeme }}
                  </p>
                </div>
    ...
    <div v-if="showSolutionCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div class="w-full max-w-lg bg-white dark:bg-gray-900 rounded-xl shadow-xl p-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Incident : <span class="font-medium text-gray-700 dark:text-white">{{ system.titre_systeme }}</span>
        </p>

        <TextareaInput
          v-model="system.solution"
    ...
    <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="w-full max-w-lg bg-white dark:bg-gray-900 rounded-xl shadow-xl p-6">
            <h2 class="text-lg font-semibold text-gray-800 dark:text-white mb-1">
                {{ selectedSysteme?.titre_systeme }}
            </h2>
            <p class="text-xs text-gray-400 mb-4">{{ selectedSysteme?.date_creation }}</p>

            <!-- Mode lecture -->
            <div v-if="!editMode">
                <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line min-h-[80px]">
                {{ selectedSysteme?.solution || 'Aucune solution renseignée.' }}
                </p>

                <!-- Fichier PDF si présent -->
                <div v-if="selectedSysteme?.fichier_solution" class="mt-4 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                      <p class="text-sm font-medium text-gray-800 dark:text-white">
                        {{ selectedSysteme.fichier_solution.split('/').pop() }}
                      </p>
            ...
            <div v-else>
                <PlaceholderInput
                    v-model="editData.titre_systeme"
                    label="Titre"
...
const currentPageTitle = ref('Incidents système')

const alert = useAlertNotifStore()

//  Formulaire
const system = reactive({ titre_systeme: '', solution: '' })
...
// Données
const systemes = ref<any[]>([])
const selectedSysteme = ref<any | null>(null)
const editData = reactive({ titre_systeme: '', solution: '' })

// Colonnes tableau
const columns = [
  { label: 'Titre', field: 'titre_systeme', width: '50%' },
...
const submitForm = async (avecSolution: boolean) => {
  try {
    ...
    formData.append('titre_systeme', system.titre_systeme)

    if (avecSolution) {
      formData.append('solution', system.solution)
    ...
    showSolutionCreateModal.value = false
    alert.showAlertNotif(
      "Une s'est produite lors de l'enregistrement",
      "error"
    )
  }
}
...
const openSolutionModal = (row: any) => {
  selectedSysteme.value = row
  editData.titre_systeme = row.titre_systeme
