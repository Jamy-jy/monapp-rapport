<template>
  <div>
        ...
        <div>
          ...
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            <div>
              ...
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ authStore.user?.nom }}</p>
            </div>

            <div>
              <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Prenom</p>
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ authStore.user?.prenom }}</p>
            </div>
            ...
            <div>
              ...
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ authStore.user?.role === 'tech' ? 'Technicien' : 'Administrateur' }}</p>
            </div>
    ...
    <Modal v-if="isProfileInfoModal" @fermer="isProfileInfoModal = false">
      <template #body>
        <div
        ...
        >
          ...
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Modification de l'information personnel
            </h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Toute modification seront appliqué immediatement. 
            </p>
          </div>
          <form class="flex flex-col" @submit.prevent="saveProfile">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  <div class="col-span-2">
                    ...
                    <input
                      type="text"
                      readonly="true"
                      :value="authStore.user?.role === 'tech' ? 'Technicien' : 'Administrateur'"
                      class="dark:bg-dark-900 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                    />
                  </div>
                </div>
            ...
            <!-- Message succès -->
            <p v-if="successMsg" class="px-2 mt-3 text-sm text-green-500">{{ successMsg }}</p>
...
const openModal = () => {
  formData.nom = authStore.user?.nom || ''
  formData.prenom = authStore.user?.prenom || ''
...
const saveProfile = async () => {
  ...
  if (formData.phone) {
    ...
    if (raw.startsWith('+261')){
      ...
      if (!/^\d*$/.test(digits)) {
        errors.phone = 'Uniquement des chiffres après +261'
        return
      }
